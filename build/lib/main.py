...
def upload_files(
    ...
    resume: bool = False,
    state_file: str = ".upload_state",
    delete: bool = False,
    output: str | None = None,
    output_format: str = "csv",
) -> list[dict]:
    files = collect_files(paths, extensions, min_size, max_size, verbose)
    if not files:
        return []
    ...
    resolved_key = api_key or os.environ.get("PILLOWS_API_KEY")
    if not resolved_key and not dry_run:
        raise ValueError("API key required. Pass api_key or set PILLOWS_API_KEY.")

    results = []
    sessions = [requests.Session() for _ in range(max(concurrency, 1))]
    progress = not no_progress and not quiet
    overall_start = time.time()
    ...
    try:
        if concurrency > 1:
            with ThreadPoolExecutor(max_workers=concurrency) as pool:
                futures = {pool.submit(do_upload, f, sessions[i % len(sessions)]): f for i, f in enumerate(files)}
                for future in as_completed(futures):
                    try:
                        result = future.result()
                        if result:
    ...
    if output and output_format != "none" and results:
        out = Path(output)
    ...
    if state:
        for r in results:
            state.record(
                ...
                size=r["size"],
                sha256=r["sha256"],
                parts_uploaded=r.get("parts_uploaded", 0),
